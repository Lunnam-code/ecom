    ...
    if request.method == 'POST':
        ...
        if user is not None:
            try:
               ...
               is_cart_item_exists = CartItem.objects.filter(cart=cart).exists()#will return True or False
            ...
            #import requests
            url = request.META.get('HTTP_REFERER')#it will grab the previous url from where you came
            try:
